		...
		void UpDate(
			...
			//射撃関連
			{
				...
				if (this->key_.drop_) {
					...
					for (auto& g : item) {
						if (g.Set_item_2(&meds_data[0], this->base.mazzule_pos(), add_vec_t, this->mat_mag)) {
							tt = true;
							break;
						}
					}
					if (!tt) {
						item.resize(item.size() + 1);
						item.back().Set_item_3(item.size() - 1, &meds_data[0], this->base.mazzule_pos(), add_vec_t, this->mat_mag);
					}
				}
				//マガジン整頓
