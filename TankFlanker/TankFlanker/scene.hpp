...
namespace FPS_n2 {
	...
	public:
		...
		public:
			...
			void Load_Human(std::shared_ptr<PLAYERclass::PLAYER_CHARA>& mine_t) {
				for (auto& s : mine_t->gun_stat) {
					for (auto& t : this->gun_stat) {
						if (s.ID == t.ID) {
							mine_t->Set_Gun_().Copy_Gun(mine_Gun, GunPartses, s);
							return;
						}
					}
				}
