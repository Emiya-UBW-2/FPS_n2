...
namespace FPS_n2 {
	...
	//
			...
			//腕のフレーム操作
			void move_hand(const bool isusewaist, const MATRIX_ref& m_inv, bool LorR) noexcept {
				//左
				if (LorR) {
					//基準
				...
				if (useVR) {
					...
					{}
					{
						...
						{}
						{
							...
							move_hand(DrawPts->tracker_num.size() > 0, m_inv, false);
							//左手
							...
							move_hand(DrawPts->tracker_num.size() > 0, m_inv, true);
							//指
